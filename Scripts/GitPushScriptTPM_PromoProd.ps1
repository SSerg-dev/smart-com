...
$path_to_repo = "D:\GitProduction\TPM_Promo"

# это нужно только для клонирования:
#If (Test-Path "D:\GitProduction\DAP") {
...
#git clone http://root:Passw0rd@10.32.95.77/Smartcom/TPM_Promo.git --depth 1 -b prod -q

if (Test-Path $path_to_repo) {
	cd $path_to_repo

    Write-host "Pull the depth 1 repository $(Get-Date -Format 'H:mm')"
	git reset --hard origin/prod > $null # без этой строчки, следующая строчка сможет перезапись в "локальном", только если на "удалённом" файл редактировался после последнего Pull (!)
    git pull http://root:Passw0rd@10.32.95.77/Smartcom/TPM_Promo.git --depth 1 prod -q --allow-unrelated-histories -s recursive -X theirs > $null 
	...
	#Get-Childitem "D:\Agent01\_work\29\s\" -Exclude "*.log", ".tfignore" | Copy-Item -Recurse
