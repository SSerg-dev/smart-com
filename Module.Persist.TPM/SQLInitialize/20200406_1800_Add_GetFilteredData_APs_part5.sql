...
DELETE FROM AccessPointRole WHERE AccessPointId IN (SELECT Id FROM ACCESSPOINT WHERE [Resource] IN ('PreviousDayIncrementals') AND [Action] IN ('GetFilteredData'));
DELETE FROM AccessPoint WHERE [Resource] IN ('PreviousDayIncrementals') AND [Action] IN ('GetFilteredData') AND [Disabled] = 'false';

declare @list varchar(MAX), @i int
select @i=0, @list ='PreviousDayIncrementals,'
