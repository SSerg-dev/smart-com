...
SELECT
	...
	CASE WHEN SUM(YEE.YTD_LSV) != 0 AND SUM(YEE.YTD_LSV) IS NOT NULL THEN SUM(pp.ActualPromoLSV)/SUM(YEE.YTD_LSV) ELSE NULL END AS VodYTD,
	CASE WHEN SUM(YEE.YEE_LSV) != 0 AND SUM(YEE.YTD_LSV) IS NOT NULL THEN (SUM(pp.ActualPromoLSV) + SUM(pp.PlanPromoLSV))/SUM(YEE.YEE_LSV) ELSE NULL END AS VodYEE,
	SUM(pp.ActualPromoLSV) AS ActualPromoLSV,
	SUM(pp.PlanPromoLSV) AS PlanPromoLSV,
	/* Shopper TI */
