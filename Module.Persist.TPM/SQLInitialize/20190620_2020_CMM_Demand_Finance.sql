...
AND RoleId IN (SELECT TOP 1 Id FROM [Role] WHERE SystemName = 'CMManager' AND [Disabled] = 0)
...
SELECT NEWID(), (SELECT  Id FROM [Role] WHERE LOWER([SystemName]) = 'CMManager' AND [Disabled] = 0), Id FROM ACCESSPOINT WHERE Resource + ':' + [Action] in (
