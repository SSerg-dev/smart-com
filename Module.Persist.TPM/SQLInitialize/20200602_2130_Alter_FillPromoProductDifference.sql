...
BEGIN
			...
			INNER JOIN [dbo].[Dates] d (NOLOCK)
				ON pdi.Week = d.MarsWeekFullName
					...
					(
						...
						FROM Dates 
						WHERE 
							OriginalDate <= CAST(GETDATE() AS DATE)
							AND MarsDay = 1
	...
	BEGIN
		...
		BEGIN		
			IF (@CDI_QTY = 0 AND @PDI_QTY IS NULL)
				OR (@CDI_QTY = 0 AND @PDI_QTY = 0)
				OR (@PDI_QTY = 0 AND @CDI_QTY IS NULL)
				CONTINUE;
			...
			IF (@PromoDisabled = 1
				...
				OR (@PDI_QTY IS NOT NULL AND @PDI_QTY <> 0 AND @CDI_QTY = 0)
				OR @CDI_QTY IS NULL
				OR @CDI_QTY < 0)
				SET @QTY = 0;
			ELSE
