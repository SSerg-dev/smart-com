CREATE OR ALTER FUNCTION ClientDashboardGetParentGHierarchyCode(@ObjectId int)
   RETURNS NVARCHAR(100)
AS BEGIN
    DECLARE @GHierarchy AS varchar(100) = NULL;
	DECLARE @ParentId AS int;
	SET @ParentId = (SELECT ParentId FROM ClientTree WHERE ObjectId = @ObjectId AND EndDate IS NULL)
	SET @GHierarchy = (SELECT GHierarchyCode FROM ClientTree WHERE ObjectId = @ObjectId AND EndDate IS NULL)
	WHILE (@GHierarchy IS NULL AND @ParentId IS NOT NULL AND @ParentId != 5000000) BEGIN
		SET @GHierarchy = (SELECT GHierarchyCode FROM ClientTree WHERE ObjectId = @ParentId AND EndDate IS NULL)
		SET @ParentId = (SELECT ParentId FROM ClientTree WHERE ObjectId = @ParentId AND EndDate IS NULL)
	END;
...
CREATE OR ALTER FUNCTION ClientDashboardGetYTDwithShare(@value AS float, @share AS float)
   RETURNS float	
AS BEGIN
    DECLARE @result AS float;
	IF ((@value IS NOT NULL) AND (@share IS NOT NULL)) SET @result = @value * @share / 100
	ELSE SET @result = 0;
	RETURN @result;
END
GO
...
SELECT
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share)) != 0 
		...
		((SUM(pp.ActualPromoTIShopper) + SUM(pp.PlanPromoTIShopperFinished))/dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share))) * 100
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)) != 0
	 AND SUM(pp.ActualPromoTIShopper) + SUM(pp.PlanPromoTIShopper) != 0 AND SUM(pp.ActualPromoTIShopper) + SUM(pp.PlanPromoTIShopper) IS NOT NULL) THEN 
		((SUM(pp.ActualPromoTIShopper) + SUM(pp.PlanPromoTIShopper))/(dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)))) * 100
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share)) != 0)
		THEN (CASE WHEN (SUM(pp.ActualPromoTICost) + SUM(pp.PlanPromoTICostFinished) IS NULL) 
					THEN 0 ELSE SUM(pp.ActualPromoTICost) + SUM(pp.PlanPromoTICostFinished) END
				+ CASE WHEN (SUM(np.ActualCostTE) + SUM(np.PlanCostTEYTD) IS NULL) 
					THEN 0 ELSE SUM(np.ActualCostTE) + SUM(np.PlanCostTEYTD) END) / dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share)) ELSE 0 END) AS MarketingTiYTDPercent,
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)) != 0)
		THEN (CASE WHEN (SUM(pp.ActualPromoTICost) + SUM(pp.PlanPromoTICost) IS NULL) 
					THEN 0 ELSE SUM(pp.ActualPromoTICost) + SUM(pp.PlanPromoTICost) END
				+ CASE WHEN (SUM(np.ActualCostTE) + SUM(np.PlanCostTEYEE) IS NULL) 
					THEN 0 ELSE SUM(np.ActualCostTE) + SUM(np.PlanCostTEYEE) END) / dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)) ELSE 0 END) AS MarketingTiYEEPercent,
	/* Production */
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share)) != 0
		...
		((SUM(pp.ActualPromoCostProduction) + SUM(pp.PlanPromoCostProductionFinished))/dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share)) * 100)
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)) != 0
		...
		((SUM(pp.ActualPromoCostProduction) + SUM(pp.PlanPromoCostProduction))/(dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)))) * 100
	...
	AND (SUM(pp.ActualPromoBranding) + SUM(pp.PlanPromoBrandingFinished)) != 0 AND (SUM(pp.ActualPromoBranding) + SUM(pp.PlanPromoBrandingFinished)) IS NOT NULL) THEN
		((SUM(pp.ActualPromoBranding) + SUM(pp.PlanPromoBrandingFinished))/dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share))) * 100
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)) != 0
		...
		((SUM(pp.ActualPromoBranding) + SUM(pp.PlanPromoBranding))/(dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)))) * 100
	...
	AND (SUM(pp.ActualPromoBTL) + SUM(pp.PlanPromoBTLFinished)) != 0
		...
		((SUM(pp.ActualPromoBTL) + SUM(pp.PlanPromoBTLFinished))/dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share))) * 100
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)) != 0
		...
		((SUM(pp.ActualPromoBTL) + SUM(pp.PlanPromoBTL))/dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YEE_LSV), SUM(SHARES.Share)) * 100)
	...
	AND (SUM(pp.ActualPromoTICost) + SUM(pp.PlanPromoTICostFinished)) != 0 
		...
		((SUM(pp.ActualPromoTICost) + SUM(pp.PlanPromoTICostFinished))/dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share))) * 100
	...
	(CASE WHEN (dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share)) != 0  
		...
		((SUM(np.ActualCostTE) + SUM(np.PlanCostTEYTD))/dbo.ClientDashboardGetYTDwithShare(SUM(YEE.YTD_LSV), SUM(SHARES.Share))) * 100
