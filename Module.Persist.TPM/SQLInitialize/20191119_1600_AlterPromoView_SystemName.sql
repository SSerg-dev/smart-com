ALTER VIEW[PromoView] AS
SELECT  pr.Id, pr.Name, mmc.Name AS MarsMechanicName, mmt.Name AS MarsMechanicTypeName, pr.MarsMechanicDiscount, cl.SystemName AS ColorSystemName, ps.Color AS PromoStatusColor,
    ps.SystemName AS PromoStatusSystemName, ps.Name AS PromoStatusName, pr.CreatorId, pr.ClientTreeId, pr.BaseClientTreeIds, pr.StartDate, DATEADD(SECOND, 86399, pr.EndDate) AS EndDate, pr.DispatchesStart,
        pr.CalendarPriority, pr.Number, bt.Name AS BrandTechName, ev.Name AS EventName, pr.InOut, pt.SystemName as TypeName, pt.Glyph as TypeGlyph 
