CREATE OR ALTER VIEW [dbo].[PlanIncrementalReport]
AS
WITH
	joined
AS
(
	SELECT 
		...
		Max(dt.OriginalDate)					AS WeekEndDate,
		Min(dt.MarsDay)							AS StartDay,
		Max(dt.MarsDay)							AS EndDay,
		Min(CAST(dt.MarsYear AS NVARCHAR(4)) + 'P' + REPLACE(STR(dt.MarsPeriod, 2), SPACE(1), '0') + dt.MarsWeekName)
												AS [Week],
...
SELECT 
	...
	[dbo].[WeekIncrementalQTY](joined.PlanProductIncrementalCaseQty,
							...
							p.DispatchesEnd,
							p.DispatchDuration,
							joined.WeekStartDate,
							joined.WeekEndDate,
							ISNULL(p.DeviationCoefficient, 0),
							joined.StartDay,
							joined.EndDay)  AS PlanProductIncrementalCaseQty,
	...
	IIF((SELECT COUNT(*) FROM Dates WHERE OriginalDate = joined.WeekStartDate AND MarsDay = 1) > 0, 
		joined.[Week], 
		(SELECT TOP(1) 
			CAST(MarsYear AS NVARCHAR(4)) + 'P' + REPLACE(STR(MarsPeriod, 2), SPACE(1), '0') + MarsWeekName
		FROM Dates WHERE OriginalDate < joined.WeekStartDate AND OriginalDate > DATEADD(DAY, -7, joined.WeekStartDate) AND MarsDay = 1))
											AS [Week],
	ps.[Name]									AS [Status]
FROM joined
	LEFT JOIN Promo p
