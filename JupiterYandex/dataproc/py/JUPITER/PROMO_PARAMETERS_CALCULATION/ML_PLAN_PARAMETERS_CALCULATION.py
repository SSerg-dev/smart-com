...
mlClientTreeId = es.input_params.get("MlClientTreeId")
...
PLAN_PROMO_PARAMETERS_CALCULATION_RESULT_PATH = SETTING_PROCESS_DIR + '/Promo/Promo.csv'
PLAN_PROMOPRODUCT_PARAMETERS_CALCULATION_RESULT_PATH = SETTING_PROCESS_DIR + '/PromoProduct/PromoProduct.csv'
PLAN_PROMOSUPPORTPROMO_PARAMETERS_CALCULATION_RESULT_PATH = SETTING_PROCESS_DIR + '/PromoSupportPromo/PromoSupportPromo.csv'
...
filteredPromoDF = promoDF.where(col('ML') == 'true').where(col('ClientTreeId') == mlClientTreeId)
...
resultPromoDF = resultPromoDF.where(col('ML') == 'true').where(col('ClientTreeKeyId') == mlClientTreeId) #??
resultPromoProductDF = resultPromoProductDF\
  .join(resultPromoDF, resultPromoDF.Id == resultPromoProductDF.PromoId, 'inner')\
  .select(\
           resultPromoProductDF['*']
         )

resultPromoProductDF.coalesce(6).write.mode("overwrite").csv(PLAN_PROMOPRODUCT_PARAMETERS_CALCULATION_RESULT_PATH)
resultPromoDF.write.mode("overwrite").csv(PLAN_PROMO_PARAMETERS_CALCULATION_RESULT_PATH)
