...
def update_baseline(parameters:dict):
    odbc_hook = OdbcHook(MSSQL_CONNECTION_NAME)
    schema = parameters["Schema"]
    result = odbc_hook.run(sql=f"""exec [{schema}].[UpdateIncreaseBaseline]""")
    print(result)
...
def add_new_baseline(parameters:dict):
    odbc_hook = OdbcHook(MSSQL_CONNECTION_NAME)
    schema = parameters["Schema"]
    result = odbc_hook.run(sql=f"""exec [{schema}].[AddNewIncreaseBaseline]""")
    print(result)
...
with DAG(
    dag_id='jupiter_update_increase_baseline',
...
# Get dag parameters from vault    
    ...
    dis_increase_baseline >> truncate_temp_increase_baseline1 >> upload_increase_baseline >> up_baseline >> truncate_temp_increase_baseline2 >> upload_new_increase_baseline >> add_new_bs >> enable_increase_baseline
