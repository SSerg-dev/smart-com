Ext.define('App.controller.tpm.promolinked.PromoLinked', {
    ...
    onPromoSupportLinkedGridAfterrender: function (grid) {
        ...
        if (!TpmModes.isRsRaMode()) {
            ...
            promoSupportPromoGridStoreProxy.extraParams.TPMmode = TpmModes.getSelectedMode().alias;
        }
    ...
    showChoosePromoWindow: function (promoSupportId, promoSupportGrid, promoLinkedGrid) {
        ...
        choosePromoWindowGrid.getStore().getProxy().extraParams.TPMmode = TpmModes.getSelectedModeId();
    ...
    onApplyActionButtonClick: function (button) {
        ...
        if (window.promoSupportId && window.promoSupportGrid) {
            ...
            $.ajax({
               type: "POST",
               cache: false,
               url: "/odata/PromoSupportPromoes/PromoSuportPromoPost?promoSupportId=" + window.promoSupportId + '&TPMmode=' + TpmModes.getSelectedModeId(),
            ...
            function onMsgBoxClose(buttonId) {
                if (buttonId === 'yes') {
                    ...
                    $.ajax({
                        type: "POST",
                        cache: false,
                        url: "/odata/PromoSupportPromoes/PromoSupportPromoDelete?key=" + record.data.Id + '&TPMmode=' + TpmModes.getSelectedModeId(),
                        dataType: "json",
                        contentType: false,
                        processData: false,
                        success: function (response) {
