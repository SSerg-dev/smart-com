...
Ext.define("Sch.tooltip.HoverTip", {
        ...
        if (f.getTarget("." + a.timeCellCls, 5) && !f.getTarget(a.eventSelector)) {
            ...
            if (d) {
                ...
                if (d - c.lastTime !== 0 || b !== c.lastResource) {
                    ...
                    if (c.hidden) {
                        c[Sch.util.Date.compareUnits(a.getTimeResolution().unit, Sch.util.Date.DAY) >= 0 ? "addCls" : "removeCls"]("sch-day-resolution");
                        c.show()
                    }
                }
            } else {
    ...
    updateHoverTip: function (c, d) {
        if (c) {
            ...
            if (this.schedulerView.getFormattedDate(c) === '0:00') {
                b = this.clockTpl.apply({
                    date: c,
                    text: this.getText(c, d)
                });
...
Ext.define("Sch.crud.AbstractManager", {
    ...
    getStore: function (b) {
        ...
        return this.storesIndex[b.storeId || b]
    },
    addStore: function (m, h, a) {
        ...
        for (var f = 0, c = m.length; f < c; f++) {
            ...
            g = g && g.prototype;
            if (!o.phantomIdField) {
                o.phantomIdField = g && g.phantomIdField
            }
            if (!o.idProperty) {
    ...
    hasChanges: function (b) {
        ...
        if (b) {
            ...
            return Boolean(c.store.getModifiedRecords().length || c.store.getRemovedRecords().length)
        }
        for (var d = 0, a = this.stores.length; d < a; d++) {
...
Ext.define("Sch.feature.DragCreator", {
    requires: ["Ext.XTemplate", "Sch.util.Date", "Sch.util.ScrollManager", "Sch.util.DragTracker", "Sch.tooltip.Tooltip", "Sch.tooltip.HoverTip"],
    disabled: false,
    showHoverTip: true,
    showDragTip: true,
    dragTip: null,
    ...
    onDragStart: function () {
        ...
        b.show();
        a.fireEvent("dragcreatestart", a, b);
        if (c.showDragTip) {
            c.dragTip.enable();
    ...
    onSchedulerDestroy: function () {
        if (this.hoverTip && this.hoverTip.rendered) {
...
Ext.define("Sch.feature.SchedulerDragZone", {
    ...
    getDragData: function (q) {
        ...
        var f = {
            ...
            resourceRecord: b,
            sourceDate: o.getDateFromCoordinate(this.getCoordinate(h)),
            origStart: c,
            origEnd: m,
            startDate: c,
            endDate: m,
...
Ext.define("Sch.feature.ResizeZone", {
    ...
    createResizer: function (b, e, o) {
        ...
        if (Ext.versions.extjs.isGreaterThanOrEqual("4.2.3.1477")) {
    ...
    partialResize: function (n, q, o, t) {
        var l = this.schedulerView,
            f = t.type === "scroll" ? this.resizer.resizeTracker.lastXY : t.getXY(),
            m = this.getStartEndDates(f),
            d = m.start,
            c = m.end,
            b = n.eventRecord,
            k = l.getMode(),
...
Ext.define("Sch.feature.Grouping", {
    ...
    constructor: function (b) {
        ...
        Ext.apply(b, {
            groupTpl: a.concat({
                priority: 200,
                shouldRenderCustomCells: function (c) {
                ...
                syncRowHeights: function (f, k) {
                    ...
                    k = Ext.fly(k, "sycSrc");
                    var d = this.owner,
                        g = f.down(d.eventSelector, true),
                        h, i = f.down(d.summaryRowSelector, true),
                        e, c, j;
                ...
                syncContent: function (d, i) {
                    ...
                    i = Ext.fly(i, "sycSrc");
                    var c = this.owner,
                        f = d.down(c.eventSelector, true),
                        e = i.down(c.eventSelector, true),
                        h = d.down(c.summaryRowSelector, true),
                        g = i.down(c.summaryRowSelector, true);
                    if (f && e) {
...
Ext.define("Sch.eventlayout.Horizontal", {
    ...
    applyLayout: function (a, c) {
        ...
        // ƒобавл€ем дополнительную строку дл€ удобства выделени€ области.
        // ≈сли записей нет (строка пуста€), не нужно добавл€ть доп строку
        var needAdditionRow = d.length > 0;
        var countOfRows = this.layoutEventsInBands(0, d);
        return this.nbrOfBandsByResource[c.internalId] = needAdditionRow ? countOfRows + 1 : countOfRows
    },
    // TODO: override in hack.js
    sortEvents: function (f, d) {
        var g = f.getStartDate(),
            i = f.getEndDate(),
            p1 = f.get('CalendarPriority');
        var e = d.getStartDate(),
            h = d.getEndDate(),
            p2 = d.get('CalendarPriority');
        var c = (g - e === 0);
        var p = (p1 === p2);
        if (c) {
            if (p) {
            ...
            // ≈сли промо не пересекаетс€ сортируем по датам
...
Ext.define("Sch.eventlayout.Table", {
    ...
    layoutEventsInBands: function (g, i) {
        var f = this.view;
        var e = f.timeAxis;
        do {
            var a = i[0],
                h = g === 0 ? f.barMargin : (g * this.timeAxisViewModel.rowHeightHorizontal - (g - 1) * f.barMargin);
            if (h >= f.cellBottomBorderWidth) {
                h -= f.cellBottomBorderWidth
            }
            while (a) {
                var d = Math.floor(e.getTickFromDate(a.start));
                var c = this.timeAxisViewModel.getPositionFromDate(e.getAt(d).getStartDate());
                var b = this.timeAxisViewModel.getTickWidth();
                a.top = h;
                a.left = c;
                a.width = b;
                Ext.Array.remove(i, a);
                a = this.findClosestSuccessor(a, i)
            }
            g++
        } while (i.length > 0);
        return g
    ...
    Ext.define("Sch.view.model.TimeAxis", {
        ...
        consumeViewPreset: function (a) {
            ...
            Ext.apply(this, {
                ...
                calendarColumnWidth: a.calendarColumnWidth || 100
            });
...
Ext.define("Sch.mixin.Lockable", {
    ...
    patchBufferedRenderingPlugin: function (c) {
        ...
        if (Ext.getVersion("extjs").isLessThan("4.2.1.883")) {
            ...
            var b = c.stretchView;
            c.stretchView = function (f, e) {
                var h = this,
                    g = (h.store.buffered ? h.store.getTotalCount() : h.store.getCount());
                if (g && (h.view.all.endIndex === g - 1)) {
                    e = h.bodyTop + f.body.dom.offsetHeight
                }
                b.apply(this, [f, e])
            }
        } else {
        ...
        var d = c.onRangeFetched;
        c.onRangeFetched = function () {
            d.apply(c, arguments);
            c.view.fireEvent("rangefetched", c, arguments)
        }
    },
...
Ext.define("Sch.model.Range", {
    ...
    getDates: function () {
        var c = [],
            b = this.getEndDate();
        for (var a = Sch.util.Date.clearTime(this.getStartDate(), true) ; a < b; a = Sch.util.Date.add(a, Sch.util.Date.DAY, 1)) {
            c.push(a)
        }
        return c
    },
...
if (!Ext.ClassManager.get("Sch.model.Resource")) {
    Ext.define("Sch.model.Resource", {
        ...
        getEventsInInterval: function (d, startDate, endDate, allowpartitial) {
            var f = this.getId() || this.internalId;
            d = d || this.getEventStore();
            var events = d.getEventsInTimeSpan(startDate, endDate, allowpartitial);
                var c = [];
                for (var b = 0, a = events.getCount() ; b < a; b++) {
                    var e = events.getAt(b);
                    if (e.data[e.resourceIdField] === f) {
                        c.push(e);
                    }
                }
...
Ext.define("Sch.data.mixin.EventStore", {
    ...
    getEventsForResourceInInterval: function (a, startDate, endDate, allowPartitial) {
        return a.getEventsInInterval(this, startDate, endDate, allowPartitial)
    },
    append: function (a) {
        ...
        if (this.fireEvent("beforereconfigure", this, b, f) !== false) {
            ...
            do {
                ...
                if (this.visibleTickStart >= 1) {
                    this.adjustedStart = c.getNext(this.adjustedStart, j, k)
                }
...
Ext.define("Sch.view.Calendar", {
    ...
    getScheduleRegion: function (d, f) {
        var h = this.view,
            g = d ? this.getColumnByResource(d).getRegion() : h.getTableRegion(),
            e = h.timeAxis.getStart(),
            k = h.timeAxis.getEnd(),
            a = h.getDateConstraints(d, f) || {
                start: e,
            ...
            c = this.translateToPageCoordinate(0),
            j = this.translateToPageCoordinate(h.getStore().getCount() * h.getRowHeight()),
            b = g.left + h.barMargin,
...
Ext.define("Sch.plugin.exporter.AbstractExporter", {
    ...
    constructor: function (a) {
        ...
        b.initConfig(a);
        if (!a.tableSelector) {
            b.tableSelector = "." + Ext.baseCSSPrefix + (Ext.versions.extjs.isLessThan("5.0") ? "grid-table" : "grid-item-container")
        }
        if (!a.name) {
            b.setName(b.L("name"))
        }
    ...
    collectRows: function (g, h, j) {
        ...
        if (f.isBuffered()) {
            if (c + 1 < d) {
                new Ext.util.DelayedTask(function () {
                    f.scrollTo(c + 1, function () {
                        f.collectRows(c + 1, h, j)
                    }, f)
                }, f).delay(50)
            } else {
                f.scrollTo(0, function () {
    ...
    preparePageToCommit: function () {
        ...
        j(d[1]);
        j(b(f.headerCt.id));
        Ext.Array.each([b(f.id), b(f.body.id), b(f.getView().id)], function (k) {
            if (k) {
                k.style.height = "100%";
                k.style.width = "100%"
            }
    ...
    scrollTo: function (a, c) {
        var b = this;
        b.lockedView.bufferedRenderer.scrollTo(a, false, function () {
            b.normalView.bufferedRenderer.scrollTo(a, false, c)
        })
    },
    ...
    restoreInfiniteScroll: function (b) {
        ...
        if (b.infiniteScroll && a.rendered) {
            ...
            a.el.scrollTo("left", this._oldScrollX);
    ...
    suspendInfiniteScroll: function (b) {
        ...
        if (b.infiniteScroll && a.rendered) {
            ...
            this._oldScrollX = a.getScroll().left;
...
Ext.define("Sch.plugin.exporter.SinglePage", {
    ...
    preparePageToCommit: function () {
        var e = this,
            f = e.callParent(arguments),
            d = f.select(".sch-secondary-canvas").first(),
            c = d.select(".sch-zone"),
            b = d.select(".sch-column-line");
...
Ext.define("Sch.plugin.Export", {
    ...
    constructor: function (a) {
        ...
        if (a.exportDialogConfig) {
            Ext.Object.each(b.defaultConfig, function (d, c, f) {
...
Ext.define("Sch.plugin.SimpleEditor", {
    ...
    onDragCreateEnd: function (c, b, d, f, a) {
        ...
        if (b.get(this.dataIndex) === "") {
            b.set(this.dataIndex, this.L("newEventText"))
        }
...
Ext.define("Sch.plugin.CellPlugin", {
    ...
    init: function (a) {
        ...
        b.lockedView.getSelectionModel().enableKeyNav = false;
        ...
        if (b.view.bufferedRenderer) {
            ...
            b.mon(b.view, "rangefetched", b.onBufferedUpdate, b)
        }
    ...
    onViewScroll: function () {
        var b = this;
        if (b.containerEl) {
            var a = Ext.fly(b.view.getNodeByRecord(b.resource));
            if (a) {
    ...
    addSelection: function () {
        var b = this;
        var a = b.lockedView.getFocusEl();
        var c = b.frameTemplate.apply({
            cls: b.frameCls,
            width: b.containerEl.getWidth(),
            height: b.containerEl.getHeight()
        });
        c = Ext.get(Ext.DomHelper.append(b.containerEl.parent(), c));
        c.setStyle("top", b.containerEl.getStyle("top"));
        c.setStyle("left", b.containerEl.getStyle("left"));
        c.removeCls("active");
        c.addCls("clone");
        a.focus();
        b.selContext.push(Ext.apply({}, b.context))
    },
    ...
    showEditorInCell: function (d, h) {
        ...
        if (d.eventRecord) {
            i.alignEditorWithRecord(d.eventRecord)
        } else {
    ...
    alignEditorWithRecord: function (a) {
        var c = this;
        var b = Ext.fly(c.view.getEventNodeByRecord(a)).getBox();
        b.y--;
        b.x--;
        c.alignEditor(b)
    },
    ...
    findPreviousIndex: function () {
        var c = this;
        var a = c.getResourceIndex();
        for (var b = a - 1; b >= 0; b--) {
            if (!c.getResource(b).hasOwnProperty("rows")) {
                return b
            }
        }
        return -1
    ...
    getCellEvents: function (b) {
        ...
        a.sortBy(function (e, d) {
            return Ext.fly(c.view.getEventNodeByRecord(e)).getY() < Ext.fly(c.view.getEventNodeByRecord(d)).getY() ? -1 : 1
        });
        return a
    },
    getAbove: function (c) {
    ...
    expandResourceRow: function (a) {
        ...
        if (b) {
            var c = Ext.fly(e.view.getEventNodeByRecord(b)).getBox();
    ...
    beginEdit: function () {
        ...
        d.prevFocusEl = d.lockedView.getFocusEl();
        d.editing = true;
        ...
        if (c) {
            var b = Ext.Date;
            var f = Ext.isArray(d.editor.dateFormat) ? d.editor.dateFormat[0] : d.editor.dateFormat;
            var a = b.format(c.getStartDate(), f);
            var e = b.format(c.getEndDate(), f);
            d.editor.record = c;
            d.editor.setValue([a, e].join(d.editor.divider));
            d.editor.recordNode = d.view.getEventNodeByRecord(c);
    ...
    completeEdit: function () {
        ...
        if (h && g.editor.isValid()) {
            ...
            if (d) {
                ...
                var f = Ext.ModelManager.create({
                    StartDate: b,
                    EndDate: e,
                    ResourceId: g.context.resource.getId()
                }, g.view.eventStore.model);
    ...
    stopEditing: function () {
        var a = this;
        if (a.editor.recordNode) {
            Ext.fly(a.editor.recordNode).show();
            delete a.editor.recordNode
        }
        a.collapseResourceRow();
        a.editor.setValue("");
        a.editing = false;
        a.clearSelection();
        a.containerEl.select(".sch-cellplugin-border").show();
        a.containerEl.setStyle("z-index", "auto");
        a.editor.hide();
        a.prevFocusEl.focus()
    },
    ...
    createFields: function () {
        ...
        if (a.showHeaderField) {
            e.showHeaderField = new Ext.form.field.Checkbox({
                ...
                name: "showHeader",
                checked: !!a.defaultConfig.showHeader,
                checkedValue: 1,
                uncheckedValue: 0
            })
        }
        if (a.showFooterField) {
            e.showFooterField = new Ext.form.field.Checkbox({
                ...
                name: "showFooter",
                checked: !!a.defaultConfig.showFooter,
                checkedValue: 1,
                uncheckedValue: 0
            })
        }
        ...
        var d = a.defaultConfig.orientation === "portrait" ? 'class="sch-none"' : "",
            c = a.defaultConfig.orientation === "landscape" ? 'class="sch-none"' : "";
...
Ext.define("Sch.mixin.AbstractSchedulerView", {
    ...
    generateTplData: function (d, c, g) {
        var f = this[this.mode].getEventRenderData(d, c, g),
            h = d.getStartDate(),
            b = d.getEndDate(),
            a = d.getCls() || "";
...
Ext.define("Sch.mixin.SchedulerView", {
    ...
    mixins: ["Sch.mixin.Localizable"],
    requires: ["Sch.tooltip.Tooltip", "Sch.feature.DragCreator", "Sch.feature.DragDrop", "Sch.feature.ResizeZone", "Sch.column.Resource", "Sch.view.Calendar", "Ext.XTemplate"],
    ...
    onRowUpdated: function (a) {
        if (this.getMode() === "horizontal") {
            if (this.hasListener("eventrepaint")) {
                Ext.each(a.getEvents(this.eventStore), function (c) {
    ...
    Ext.define("Sch.mixin.TimelinePanel", {
        ...
        mainRenderer: function (b, m, g, j, l) {
            var c = this.renderers,
                k = this.mode === "horizontal",
                d = this.mode === "horizontal" || this.mode === "calendar" ? g : this.resourceStore.getAt(l),
                a = "&nbsp;";
            m.rowHeight = null;
        ...
        setupPartnerTimelinePanel: function () {
            ...
            if (f.lockedGrid.getCollapsed()) {
                f.lockedGrid.on("viewready", function (g) {
                    this.lockedGrid.setWidth(g.getWidth())
                }, this)
            } else {
...
Ext.define("Sch.mixin.SchedulerPanel", {
    ...
    refreshResourceColumns: function () {
        ...
        this.lockedGrid.view.saveScrollState();
        this.normalGrid.view.saveScrollState();
        this.normalGrid.reconfigure(null, this.createResourceColumns(a));
        this.lockedGrid.view.restoreScrollState();
        this.normalGrid.view.restoreScrollState()
    },
