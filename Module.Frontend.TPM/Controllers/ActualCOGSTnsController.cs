...
namespace Module.Frontend.TPM.Controllers
{
    public class ActualCOGSTnsController : EFContextController
    {
        ...
        [ClaimsAuthorize]
        public async Task<IHttpActionResult> Put([FromODataUri] Guid key, Delta<ActualCOGSTn> patch)
        {
        ...
        [ClaimsAuthorize]
        public async Task<IHttpActionResult> Post(ActualCOGSTn model)
        {
            ...
            var configuration = new MapperConfiguration(cfg =>
                cfg.CreateMap<ActualCOGSTn, ActualCOGSTn>().ReverseMap());
        ...
        public async Task<IHttpActionResult> Patch([FromODataUri] Guid key, Delta<ActualCOGSTn> patch)
        {
            try
            {
                var model = Context.Set<ActualCOGSTn>().Find(key);
        ...
        [ClaimsAuthorize]
        public async Task<IHttpActionResult> CreateActualCOGSTnChangeIncidents()
        {
            var previousYear = DateTimeOffset.Now.AddYears(-1).Year;
            var settingsManager = (ISettingsManager)IoC.Kernel.GetService(typeof(ISettingsManager));
            var statusesSetting = settingsManager.GetSetting<string>("ACTUAL_COGSTI_CHECK_PROMO_STATUS_LIST", "Finished,Closed");
        ...
        [ClaimsAuthorize]
        public async Task<IHttpActionResult> ExportXLSX(ODataQueryOptions<ActualCOGSTn> options)
        {
        ...
        public async Task<HttpResponseMessage> FullImportXLSX()
        {
            try
            {
                ...
                await CreateImportTask(fileName, "FullXLSXCOGSTnUpdateImportHandler", form);
