...
namespace Module.Frontend.TPM.Controllers
{
    public class HistoricalPromoTypesController : ODataController
    {
        ...
        [ClaimsAuthorize]
        [HttpPost]
        public IQueryable<HistoricalPromoTypes> GetFilteredData(ODataQueryOptions<HistoricalPromoTypes> options)
        {
            var query = Enumerable.Empty<HistoricalPromoTypes>().AsQueryable();
            ...
            if (isArgumentExists)
            {
                ...
                query = HistoryReader.GetAllById<HistoricalPromoTypes>(id.ToString());
            }
            else
            {
                query = HistoryReader.GetAll<HistoricalPromoTypes>();
            }
            ...
            var optionsPost = new ODataQueryOptionsPost<HistoricalPromoTypes>(options.Context, Request, HttpContext.Current.Request);
            return optionsPost.ApplyTo(query, querySettings) as IQueryable<HistoricalPromoTypes>;
        }
