...
namespace Module.Frontend.TPM.Controllers {
    ...
    public class HistoricalStoreTypesController : ODataController {
        ...
        [ClaimsAuthorize]
        [HttpPost]
        public IQueryable<HistoricalStoreType> GetFilteredData(ODataQueryOptions<HistoricalStoreType> options)
        {
            var query = Enumerable.Empty<HistoricalStoreType>().AsQueryable();
            ...
            if (isArgumentExists)
            {
                ...
                query = HistoryReader.GetAllById<HistoricalStoreType>(id.ToString());
            }
            else
            {
                query = HistoryReader.GetAll<HistoricalStoreType>();
            }
            ...
            var optionsPost = new ODataQueryOptionsPost<HistoricalStoreType>(options.Context, Request, HttpContext.Current.Request);
            return optionsPost.ApplyTo(query, querySettings) as IQueryable<HistoricalStoreType>;
        }
