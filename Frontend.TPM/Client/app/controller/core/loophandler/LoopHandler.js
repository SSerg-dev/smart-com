Ext.define('App.controller.core.loophandler.LoopHandler', {
    ...
    onInterfaceManualSendButtonClick: function (button) {
        ...
        if (selModel.hasSelection()) {
            ...
            if (record.get('InterfaceDirection') !== 'OUTBOUND') {
                ...
                breeze.EntityQuery
                    ...
                    .then(function (data) {
                        ...
                        if (result.success) {
                            App.Notify.pushInfo('Задача отправки сообщения успешно создана');
            ...
            App.Notify.pushInfo('Сначала нужно выделить запись');
        }
    },
        ...
        breeze.EntityQuery
            ...
            .then(function (data) {
                ...
                if (result.success) {
                    ...
                    App.Notify.pushInfo('Задача разбора сообщения успешно создана');
                    // Открыть панель задач
    ...
    openManualExtractWindow: function (form, handlerData, resource, action) {
        ...
        var editor = Ext.create('App.view.core.common.EditorWindow', {
            ...
            successMessage: 'Задача ручного извлечения данных успешно создана',
            resource: resource,
            action: action,
    ...
    openTextWindow: function (text, wigdetName) {
        ...
        if (textField) {
            ...
            App.Notify.pushError('Ошибка при отображении окна лога');
        }
    },
