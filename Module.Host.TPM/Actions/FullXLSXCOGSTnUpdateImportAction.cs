...
namespace Module.Host.TPM.Actions
{
    ...
    /// </summary>
    public class FullXLSXCOGSTnUpdateImportAction : BaseAction
    {
        public FullXLSXCOGSTnUpdateImportAction(FullImportSettings settings, int year, string importDestination)
        {
        ...
        private ImportResultFilesModel ApplyImport(IList<IEntity<Guid>> sourceRecords, out int successCount, out int warningCount, out int errorCount)
        {
            ...
            using (DatabaseContext context = new DatabaseContext())
            {
                ...
                Parallel.ForEach(sourceRecords, item => {
                    int objId = ((ImportCOGSTn)item).ClientTreeObjectId;
                    String btName = ((ImportCOGSTn)item).BrandsegTechsub;
                    if (existedexistedClientTreesIds.Contains(objId))
                    {
                        var finden = existedClientTreesTuples.FirstOrDefault(y => y.Item2 == objId);
                        if (finden != null)
                        {
                            ((ImportCOGSTn)item).ClientTreeId = finden.Item1;
                        }
                    }
                    var bt = brandTechesTuples.FirstOrDefault(y => y.Item1 == btName);
                    ((ImportCOGSTn)item).BrandTechId = bt == null ? null : bt.Item2;
                });
