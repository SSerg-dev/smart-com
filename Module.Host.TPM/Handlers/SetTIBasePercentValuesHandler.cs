...
{
    /// <summary>
    /// Класс для запуска экшена для расчета значения TIBasePersent в промо
    /// </summary>
    public class SetTIBasePercentValuesHandler : BaseHandler
    {
        public override void Action(HandlerInfo info, ExecuteData data)
        {
            ...
            try
            {
                ...
                handlerLogger.Write(true, String.Format("TIBasePersent calculation began at {0:yyyy-MM-dd HH:mm:ss}", ChangeTimeZoneUtil.ChangeTimeZone(DateTimeOffset.UtcNow)), "Message");
            ...
            finally
            {
                ...
                if (handlerLogger != null)
                {
                    handlerLogger.Write(true, String.Format("TIBasePersent calculation ended at {0:yyyy-MM-dd HH:mm:ss}. Duration: {1} seconds", ChangeTimeZoneUtil.ChangeTimeZone(DateTimeOffset.UtcNow), sw.Elapsed.TotalSeconds), "Message");
        ...
        {
            return new SetTIBasePercentValuesAction();
        }
    }
}